<?php
  ...
  if($op == 'edicion/de/servicios') {
    ...
    $row = $objGeneral->seleccionar_registro("servicios","idServicio",$id);
    ...
    <div class="container">
      ...
      <div class="row">
        ...
        <ol class="breadcrumb">
          ...
          <a href="proceso.php?op=panel/servicios" class="btn btn-default btn-xs pull-right">Volver</a>
        </ol>
      ...
      <div class="row">
        <?php echo $result; ?>
        <form action="frm_servicios/procesar.php?action=<?php echo $action; ?>" class="form-horizontal" method="POST" enctype="multipart/form-data" role="form">
          
          <input type="hidden" name="idOwner" class="idOwner" value="<?php echo $_SESSION['idUsuario']; ?>" />
          <input type="hidden" name="idServicio" id="idServicio" value="<?php echo $row['idServicio']; ?>" />
          ...
          <div class="col-lg-9 contenido">
            ...
            <div class="form-group">
              <label>Enlace permanente:</label> <i class="glyphicon glyphicon-new-window"></i> <a href="<?php echo _global_siteurl; ?>servicio/<?php echo $row['idServicio']; ?>" target="_blank"><?php echo _global_siteurl; ?>servicio/<?php echo $row['idServicio']; ?></a>
            </div>
            <?php } ?>
          ...
          <div class="col-lg-3 sidebar">
            ...
            <div class="panel panel-info">
                ...
                <div class="panel-footer" style="text-align: right;">
                  <?php if($action == 'update'){ ?>
                    <a href="#" class="delete-row pull-left" data-id="<?php echo $row['idServicio']; ?>" alt="<?php echo $row['titulo']; ?>" title="Eliminar" style="margin-top:6px;"><i class="glyphicon glyphicon-trash"></i> Eliminar</a>
                  <?php } ?>
              ...
              <div class="panel panel-info">
                ...
                <div class="panel-body">
                  ...
                  <select class="form-control input-sm" id="categoria" name="categoria" required >
                    ...
                    <?php 
                      $categorias = $objGeneral->listar_registros("SELECT * from categorias WHERE tipo = 'servicios' AND publicada = 'SI' AND eliminado = '0'"); 
                      if($categorias)
                      {
            ...
            <div class="form-group">
              <div class="panel panel-info">
                ...
                <div class="panel-body">
                  <?php if($row['imagen'] != ''){ ?>
                    <img src="timthumb.php?src=<?php echo _global_siteurl; ?>adm-gestor/frm_servicios/img/<?php echo $row['imagen']; ?>&amp;w=300" class="" style="width:100%;">
                  <?php } else { ?>
            ...
            <div class="form-group">
              <div class="panel panel-info">
                ...
                <div class="panel-body">
                  <?php if($row['archivo'] != ''){ ?>
                    <?php if($row['archivo'] != ''){ echo '<strong><a href="frm_servicios/'. $row['archivo'] .'" target="_blank"><i class="glyphicon glyphicon-paperclip text-primary"></i> ' . $row['archivo'] . '</a></strong>' ; } ?>
                  <?php } else { ?>
          ...
          if ( confirm( "Esta seguro que desea eliminar " + nombre + "?" ) ) {
            ...
            success: function(data){
              ...
              if(data.success){
                ...
                $(location).attr('href', 'proceso.php?op=panel/servicios');                
              }
