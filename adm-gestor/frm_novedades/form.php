<?php
  ...
  if($op == 'edicion/de/novedades') {
    ...
    $row = $objGeneral->seleccionar_registro("novedades","idNovedad",$id);
    ...
    <div class="container">
      ...
      <div class="row">
        ...
        <ol class="breadcrumb">
          ...
          <a href="proceso.php?op=panel/novedades" class="btn btn-default btn-xs pull-right">Volver</a>
        </ol>
      ...
      <div class="row">
        <?php echo $result; ?>
        <form action="frm_novedades/procesar.php?action=<?php echo $action; ?>" class="form-horizontal" method="POST" enctype="multipart/form-data" role="form">
          
          <input type="hidden" name="idOwner" class="idOwner" value="<?php echo $_SESSION['idUsuario']; ?>" />
          <input type="hidden" name="idNovedad" id="idNovedad" value="<?php echo $row['idNovedad']; ?>" />
          ...
          <div class="col-lg-9 contenido">
            ...
            <div class="form-group">
              <label>Enlace permanente:</label> <i class="glyphicon glyphicon-new-window"></i> <a href="<?php echo _global_siteurl; ?>novedades/<?php echo $row['idNovedad']; ?>" target="_blank"><?php echo _global_siteurl; ?>novedades/<?php echo $row['idNovedad']; ?></a>
            </div>
            <?php } ?>
          ...
          <div class="col-lg-3 sidebar">
            ...
            <div class="panel panel-info">
                ...
                <div class="panel-footer" style="text-align: right;">
                  <?php if($action == 'update'){ ?>
                    <a href="#" class="delete-row pull-left" data-id="<?php echo $row['idNovedad']; ?>" alt="<?php echo $row['titulo']; ?>" title="Eliminar" style="margin-top:6px;"><i class="glyphicon glyphicon-trash"></i> Eliminar</a>
                  <?php } ?>
            ...
            <div class="form-group">
              <div class="panel panel-info">
                ...
                <div class="panel-body">
                  ...
                  <select class="form-control input-sm" id="categoria" name="idCategoria" required >
                    <option value="0" <?php if($row['idCategoria'] == '0'){ echo 'selected="selected"' ; } ?> >Generales</option>
                    <?php 
                      $categorias = $objGeneral->listar_registros("SELECT * from categorias WHERE tipo = 'novedad' AND publicada = 'SI' AND eliminado = '0'"); 
                      if($categorias)
                      {
                        foreach($categorias as $categoria)
                          {
                      ?>
                        <option value="<?php echo $categoria['idCategoria']; ?>" <?php if($categoria['idCategoria'] == $row['idCategoria']){ echo 'selected'; } ?> ><?php echo ucfirst($categoria['nombre']); ?></option>
                      <?php 
                          }  
                      } 
                    ?>
            ...
            <div class="form-group">
              <div class="panel panel-info">
                ...
                <div class="panel-body">
                  <?php if($row['imagen'] != ''){ ?>
                    <img src="timthumb.php?src=<?php echo _global_siteurl; ?>adm-gestor/frm_novedades/img/<?php echo $row['imagen']; ?>&amp;w=300" class="" style="width:100%;">
                  <?php } else { ?>
              ...
              <div class="panel panel-info">
                ...
                <div class="panel-body">
                  <?php if($row['archivo'] != ''){ ?>
                    ...
                    <strong><a href="frm_novedades/<?php echo $row['archivo']; ?>" target="_blank"><img src="img/files/<?php echo substr($row['archivo'], -3); ?>.png" height="30"> <?php echo $row['archivo']; ?></a></strong>
          ...
          if ( confirm( "Esta seguro que desea eliminar " + nombre + "?" ) ) {
            ...
            success: function(data){
              ...
              if(data.success){
                ...
                $(location).attr('href', 'proceso.php?op=panel/novedades');                
              }
