<?php
  ...
  $sql = $objGeneral->listar_registros("SELECT * FROM categorias WHERE eliminado = '0' ORDER BY nombre ASC");
    ...
    <div class="container">
      ...
      <div class="row">
        <div class="page-header">
          <h2>Categorias <small></small><a href="proceso.php?op=alta/de/categorias" class="btn btn-primary pull-right">Nuevo</a></h2>
        </div>
      </div>
      ...
      <div class="row">
        ...
        <table class="table table-condensed table-hover footable toggle-square" data-filter="#filter" data-page-size="10">
            ...
            <tbody>
              ...
              if($sql)
              {
                foreach($sql as $categorias)
                  {
              ?>

                <tr>
                  <td><?php echo $categorias['nombre']; ?> </td>
                  <td><?php echo ucfirst($categorias['tipo']); ?></td>
                  <td><?php if($categorias['publicada'] == 'SI'){ echo '<span class="label label-success">Publicada</span>';}else{ echo '<span class="label label-danger">Borrador</span>';} ?> </td>
                  <td>
                    <a href="proceso.php?op=edicion/de/categorias&amp;id=<?php echo $categorias['idCategoria']; ?>" class="btn btn-primary btn-xs" title="Editar"><i class="glyphicon glyphicon-pencil"></i></a>
                    <a href="#" class="btn btn-danger btn-xs delete-row" data-id="<?php echo $categorias['idCategoria']; ?>" alt="<?php echo $categorias['nombre']; ?>" title="Eliminar"><i class="glyphicon glyphicon-trash"></i></a>
                  </td>
                </tr>
